...
def process_backtest_tmp(strategy, res, threshold=10):
    u"""
        回测测试
    ...
    if not res:
        return
    day = strategy.split("__")[1]
    strategy = strategy.split("__")[0]
    success_count = 0
    fail_count = 0
    total = len(res)
    for stock in res:
        print(day,strategy,stock)
        data = utils.read_data((stock[0],stock[1]))
        is_success = process_single(day, data, threshold)
        if is_success == 0 and total > 1:
            ...
            fail_count += 1
    logging.info("day:{}, {} 在 {} 天后, 总数:{}, 成功数:{}, 失败数:{}, 成功率: {:.2%}, 失败率 {:.2%}".format(day,strategy,threshold,total,success_count,fail_count, success_count/total,fail_count/total))

def process_single(start_date, data, threshold = 5):
    try:
        ...
        if end_close / start_close >= 1.02:
            # push.sink_to_txt("{}, 预测日期:{}, 成功预测！{}天后，上涨幅度: {:.2%}\n".format(stock[1],start_date, threshold, end_close / start_close - 1))
            return 1
